...
const Search = () => {
	...
	const getCourseCountText = (count: number) => {
		if (count === 1) return "1 курс доступен";
		if (count < 5) return `${count} курса доступно`;
		return `${count} курсов доступно`;
	};

	return (
		<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="search">
			<h1 className="search__title">Каталог доступных курсов</h1>
			<h2 className="search__subtitle">{getCourseCountText(courses.length)}</h2>
