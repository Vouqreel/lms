...
const ChaptersSidebar = () => {
	...
	if (!user) return <div>Пожалуйста, войдите в систему для просмотра прогресса курса.</div>;
		...
		<div className="chapters-sidebar__section">
			<div onClick={() => toggleSection(section.sectionTitle)} className="chapters-sidebar__section-header">
				<div className="chapters-sidebar__section-title-wrapper">
					<p className="chapters-sidebar__section-number">Раздел 0{index + 1}</p>
					{isExpanded ? (
		...
		>
			{isCompleted ? (
				<div
					...
					title="Переключить статус завершения"
				>
